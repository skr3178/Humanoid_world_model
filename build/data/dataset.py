...
class HumanoidWorldModelDataset(Dataset):
    ...
    def __init__(
        self,
        data_dir: str,
        num_past_frames: int = 34,  # 2 clips * 17 frames/clip (paper default)
        num_future_frames: int = 17,  # 1 clip * 17 frames/clip (paper default)
        num_past_clips: Optional[int] = None,  # If set, overrides num_past_frames
        num_future_clips: Optional[int] = None,  # If set, overrides num_future_frames
        stride: int = 1,
        ...
        Args:
            ...
            num_past_frames: Number of past frames (default: 34 = 2 clips)
            num_future_frames: Number of future frames (default: 17 = 1 clip)
            num_past_clips: Explicit past clip count (overrides frame-based calculation)
            num_future_clips: Explicit future clip count (overrides frame-based calculation)
        ...
        # Use explicit clip counts if provided, else compute from frames
        self.num_past_clips = num_past_clips if num_past_clips is not None else math.ceil(num_past_frames / FRAMES_PER_CLIP)
        self.num_future_clips = num_future_clips if num_future_clips is not None else math.ceil(num_future_frames / FRAMES_PER_CLIP)
        self.stride = stride
        ...
        # Total clips needed per sample
        clips_needed = self.num_past_clips + self.num_future_clips
    ...
    def __getitem__(self, idx: int) -> Dict[str, torch.Tensor]:
        ...
        Per paper: 2 past latents + 1 future latent
        ...
        - actions_past: (T_p, 25) - past states at frame level
        - actions_future: (T_f, 25) - future states at frame level
        """
        ...
        # Use explicit clip counts
        past_clips = self.num_past_clips
        future_clips = self.num_future_clips
        total_clips = past_clips + future_clips

        # Extract video clip tokens
        clip_inds = list(range(start_clip, start_clip + total_clips))
        video_clips = shard_info["video_tokens"][clip_inds]  # (total_clips, 3, 32, 32)

        video_past = video_clips[:past_clips]  # (num_past_clips, 3, 32, 32)
        video_future = video_clips[past_clips:past_clips + future_clips]  # (num_future_clips, 3, 32, 32)

        # Extract states at frame level
        # Past frames: num_past_clips * 17 frames
        # Future frames: num_future_clips * 17 frames
        start_frame = start_clip * FRAMES_PER_CLIP
        total_frames = total_clips * FRAMES_PER_CLIP
        past_frame_count = past_clips * FRAMES_PER_CLIP
        future_frame_count = future_clips * FRAMES_PER_CLIP
        ...
        actions_past = states[:past_frame_count]  # (past_clips * 17, 25)
        actions_future = states[past_frame_count:past_frame_count + future_frame_count]  # (future_clips * 17, 25)
