#!/bin/bash
# Train on the full v2.0 dataset with 12GB GPU configuration
# Uses reduced model size (12 layers, 256 dim) to fit in 12GB VRAM
# Original paper config (24 layers, 512 dim) remains unchanged
...
echo "  - 12 layers (reduced from 24)"
echo "  - 4 heads (reduced from 8)"
echo "  - 256 dim (reduced from 512)"
echo "  - 1024 MLP (reduced from 2048)"
echo "  - vocab_size: 65536 (per factor) - UNCHANGED"
echo "  - action_dim: 25 (R^25) - UNCHANGED"
echo ""
echo "Training hyperparameters (per paper):"
echo "  - Batch size: 16 (single GPU)"
...
# Uses MaskedHWM12GBConfig (12 layers, 256 dim, 4 heads)
